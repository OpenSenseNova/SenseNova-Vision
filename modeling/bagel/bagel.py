...
class Bagel(PreTrainedModel):
    ...
    def forward(
        ...
        if self.config.visual_und:
            # Check if we have any visual understanding tokens
            if vit_token_seqlens is not None and len(vit_token_seqlens) > 0:
                ...
                # Dummy pass to prevent FSDP hang
                # Ensure vit_model parameters are involved in the graph even if no data
                dtype = packed_text_embedding.dtype
                device = packed_text_embedding.device

                # Create minimal dummy inputs (1 patch)
                # Input dim: patch_size * patch_size * 3 (channels)
                dim = self.vit_patch_size * self.vit_patch_size * 3
                dummy_vit_tokens = torch.zeros((1, dim), dtype=dtype, device=device)
                dummy_pos_ids = torch.zeros((1,), dtype=torch.long, device=device)
                dummy_cu_seqlens = torch.tensor([0, 1], dtype=torch.int32, device=device)
                dummy_max_seqlen = 1
                ...
                dummy_out = self.vit_model(
                    ...
                    max_seqlen=dummy_max_seqlen
                )
                dummy_out = self.connector(dummy_out)
                dummy_pos = self.vit_pos_embed(dummy_pos_ids)
                dummy_out = dummy_out + dummy_pos

                # Connect to graph by adding zero-weighted sum to packed_sequence
                # This ensures gradients (of 0) flow to vit_model parameters
        ...
        if self.config.visual_gen:
            ...
            if padded_latent is not None:
                ...
                # dummies for umm FSDP
                packed_latent_position_ids = packed_position_ids.new_zeros(size=[0])
                packed_timesteps = packed_sequence.new_zeros(size=[0])
                packed_latent = packed_sequence.new_zeros(size=[0, p * p * self.latent_channel])
            ...
            if packed_vae_token_indexes is None:
                # dummy for umm FSDP
                packed_vae_token_indexes = packed_text_indexes.new_ones(size=[0])
        ...
        if self.config.visual_gen:
            if mse_loss_indexes is None:
                ...
                target = packed_sequence.new_zeros(size=[0, self.latent_patch_size * self.latent_patch_size * self.latent_channel])
            else:
                has_mse_loss = True
        ...
        return dict(mse=mse, ce=ce, has_mse=has_mse_loss, has_ce=has_ce_loss)
