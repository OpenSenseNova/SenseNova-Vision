...
"""
Dataset classes for BAGEL training with support for 2D visual understanding.

This module provides:
- DataConfig: Configuration for 2D visual understanding (VIT + VAE)
- PackedDataset: Dataset class for 2D visual understanding
- SimpleCustomBatch: Batch class for 2D visual understanding
...
from .transforms import ImageTransform, PointmapTransform
from .video_utils import FrameSampler

LARGE_GROUPS = ['recon3d_sft', 'recon3d_jsonl_sft']
...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            ...
            if 'pointmap_vae_transform_args' in dataset_args.keys():
                pointmap_vae_transform = PointmapTransform(**dataset_args.pop('pointmap_vae_transform_args'))
                dataset_args['pointmap_vae_transform'] = pointmap_vae_transform
            if 'pointmap_vit_transform_args' in dataset_args.keys():
                pointmap_vit_transform = PointmapTransform(**dataset_args.pop('pointmap_vit_transform_args'))
                dataset_args['pointmap_vit_transform'] = pointmap_vit_transform
    ...
    def __iter__(self):
        ...
        grouped_names = [d.dataset_name for d in self.grouped_datasets]
        ...
        while True:
                    ...
                    if self.is_mandatory[group_index]:
                        while True:
                            ...
                            if num_tokens < self.max_num_tokens_per_sample:
                                # defense: avoid exceeding max_num_tokens when packing mandatory samples
                                # design target: keep mandatory sampling moving, and reduce IO pressure
                                if sequence_status['curr'] + num_tokens > self.max_num_tokens:
                                    # If current packed sequence is near full, yield it first,
                                    # then pack this mandatory sample into a fresh sequence.
                                    if sequence_status['curr'] > 0:
                                        print(f"Yielding data with length {sum(sequence_status['sample_lens'])}")
                                        data = self.to_tensor(sequence_status)
                                        data['batch_data_indexes'] = batch_data_indexes
                                        data['batch_group_name'] = batch_group_name
                                        yield data
                                        sequence_status = self.set_sequence_status()
                                        batch_data_indexes = []
                                        batch_group_name = []
                                ...
                                batch_group_name.append(grouped_names[group_index])
                                break
                            else:
            ...
            if sequence_status['curr'] < self.prefer_buffer_before and len(buffer) > 0:
                group_index, sample = buffer.pop(0)
            ...
            # if a sample is too long, skip it
            num_tokens = sample['num_tokens'] + 2 * len(sample['sequence_plan'])
            group_name = grouped_names[group_index]
            if group_name in LARGE_GROUPS and num_tokens > 32000:
                print(f"skip a {group_name} sample with length 32000")
                continue
            if group_name not in LARGE_GROUPS and num_tokens > self.max_num_tokens_per_sample:
                print(f"skip a {group_name} sample with length {num_tokens}")
                continue

            if sequence_status['curr'] + num_tokens > self.max_num_tokens:
                if len(buffer) >= self.max_buffer_size and not sample_from_buffer:
                    print(f"drop a sample of length {num_tokens}: buffer full")
                if len(buffer) < self.max_buffer_size and not sample_from_buffer:
                    buffer.append((group_index, sample))
                else:
                    # print(f"Yielding data with length {sum(sequence_status['sample_lens'])}; Hard-limit {len(sequence_status['sample_lens'])} samples.")
                    if sample_from_buffer:
                        print(f"drop a sample of length {num_tokens}: sample from buffer but exceed max tokens")
                    data = self.to_tensor(sequence_status)
                    data['batch_data_indexes'] = batch_data_indexes
                    data['batch_group_name'] = batch_group_name
                    yield data
            ...
            if sequence_status['curr'] >= self.expected_num_tokens:
                # print(f"Yielding data with length {sum(sequence_status['sample_lens'])}; Soft-limit {len(sequence_status['sample_lens'])} samples.")
                data = self.to_tensor(sequence_status)
                data['batch_data_indexes'] = batch_data_indexes
                data['batch_group_name'] = batch_group_name
                yield data
...
class SimpleCustomBatch:
    def __init__(self, batch):
        ...
        self.batch_group_name = data.get('batch_group_name', None)
    ...
    def to_dict(self):
        data = dict(
            ...
            batch_group_name = self.batch_group_name,
        )
