...
from datasets.utils.geometry import unproject_depth_map_to_point_map
...
class DTU(Dataset):
    def __init__(
        ...
        print(f"[DTU] loading from {DTU_DIR}")
    ...
    def get_data(
        ...
        for id_index, id in enumerate(ids):
            ...
            mask_pil = Image.open(maskpath)

            if mask_pil.mode in ['L', '1']:
                mask = np.array(mask_pil) / 255.0
            else:
                mask = np.array(mask_pil.convert('L')) / 255.0
