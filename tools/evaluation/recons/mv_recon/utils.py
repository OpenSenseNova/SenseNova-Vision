...
def precision(gt_points, rec_points, dist_th):
    kd_tree = KDTree(gt_points)
    distances, _ = kd_tree.query(rec_points, workers=-1)
    return np.mean((distances < dist_th).astype(np.float32))


def recall(gt_points, rec_points, dist_th):
    kd_tree = KDTree(rec_points)
    distances, _ = kd_tree.query(gt_points, workers=-1)
    return np.mean((distances < dist_th).astype(np.float32))
...
def accuracy_and_precison(gt_points, rec_points, dist_th):
    kd_tree = KDTree(gt_points)
    distances, _ = kd_tree.query(rec_points, workers=-1)
    acc = np.mean(distances)
    prec = np.mean((distances < dist_th).astype(np.float32))
    return acc, prec


def completion_and_recall(gt_points, rec_points, dist_th):
    kd_tree = KDTree(rec_points)
    distances, _ = kd_tree.query(gt_points, workers=-1)
    comp = np.mean(distances)
    rcl = np.mean((distances < dist_th).astype(np.float32))
    return comp, rcl
