...
def pq_compute_multi_core(matched_annotations_list, gt_folder, pred_folder, categories):
    if not matched_annotations_list:
        return PQStat()

    requested_workers = os.environ.get("SEG_PQ_WORKERS", "").strip()
    if requested_workers:
        try:
            cpu_num = int(requested_workers)
        except ValueError as exc:
            raise ValueError(f"SEG_PQ_WORKERS must be an integer, got: {requested_workers}") from exc
    else:
        cpu_num = min(multiprocessing.cpu_count(), 16)
    cpu_num = max(1, min(cpu_num, len(matched_annotations_list)))
